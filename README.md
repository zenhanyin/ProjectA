...
`index.html` 通过查询参数给 CSS 和 JS 加版本号，例如：

```html
css/style.css?v=20260906-p05r2
js/app.js?v=20260906-p05r2
```

后续只要发生 HTML 与 JS/CSS 结构不兼容的部署，都需要同步更新这些 `?v=` 参数，避免浏览器加载新 HTML 与旧脚本造成白屏。

## Phase 0.5 收口规则

- 工作日采用最小规则：星期一至星期五累计进度，星期六和星期日不累计。
- 工作结束时间必须晚于工作开始时间，Phase 0.5 暂不支持跨午夜工作时段。
- “本次打开以来”只在当前页面会话中按实际有效工作时间累计，不写入 LocalStorage。
- 初始化完成或修改工作/收入参数后，会话累计会重新从 0 开始。
- 专注模式下历史、我的和快捷入口不展示具体金额、账户余额、资产、负债或 `¥` 信息。
